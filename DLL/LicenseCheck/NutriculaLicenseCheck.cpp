...
namespace
{
    ...
    void RunOwnedRefresh(uint64_t generation, uint64_t ownerNonce, const std::string& licenseIdForRequest)
    {
        ...
        const std::wstring urlPath = L"/login2/license_check.php";
        long finalStable = TIER_FAILED;

        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++)
        {
            SetPending(PENDING_COMM_FAIL_RETRYING);
            g_coordinator.Heartbeat(generation, ownerNonce, static_cast<uint32_t>(attempt));
    ...
    void MaybeSendStatsPing()
    {
        ...
        Transport::PostEnvelope(L"nutriculaexpert.com", L"/login2/license_check.php", envelope, 15000);
