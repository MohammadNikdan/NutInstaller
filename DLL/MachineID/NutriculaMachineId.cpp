...
thread_local std::wstring g_lastClientIp;
...
NUTRICULA_API int __cdecl Nutricula_GetLastClientIp(char* output, int outputCapacity) {
    if (!output || outputCapacity < 1) return -1;
    output[0] = '\0';
    if (g_lastClientIp.empty()) return 0;
    std::string utf8 = ToUtf8(g_lastClientIp);
