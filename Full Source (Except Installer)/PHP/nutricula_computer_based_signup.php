...
try {
    ...
    nutricula_rate_limit_check($conn, $config, 'signup');
    ...
    /* VPS IP-Binding (see nutricula_effective_machine_id()'s own doc
       comment in license_common.php for the full design) - BUG FIX: this
       was previously computed and used ONLY in license_check.php's verify
       path, never here at signup, which meant a VPS's stored machine_id
       column held the RAW (non-IP-augmented) value while every subsequent
       verify computed and compared against the EFFECTIVE (IP-augmented)
       value - these two could structurally never match, so every VPS
       signup was silently guaranteed to fail its very first verify with
       machine_mismatch. $machineId itself stays the raw, client-reported
       value everywhere else in this file (including the signed lease
       canonical near the bottom) - only $effectiveMachineId is ever
       written to the machine_id DATABASE column. */
    ...
    // vps_bound_ip column comment in schema.sql.
    ...
    if ($existing !== null) {
        ...
        if (strcasecmp((string)$existing['user_email'], $email) !== 0 ||
            ...
            // only creates a new key if none exists yet, never forces a
        ...
        /* Re-signup with the same purchase key on the same computer is
           deliberately run through the EXACT SAME clone-detection state
           machine as a normal verify cycle - as if it had presented a
           stale/absent refresh token (which is literally true: a signup
           request carries no refresh_token at all). This means:
           - Not currently suspicious -> forgiven once: a new token is
             issued and the license is flagged suspicious (NOT reset to a
             clean state - re-signup is treated as one stale-token event,
             same as receiving an actual old token would be).
           - Already suspicious (this would be the second consecutive
             stale-like event) -> blocked for clone_block_hours, exactly
             like two consecutive stale tokens would trigger.
