...
CREATE TABLE nutricula_unlicensed_checkins (
    ...
    /* Both nullable, but at least one is always present (enforced in PHP,
       ...
       generate on some systems (see NutriculaMachineId's own acceptance
       logic), in which case device_public_key_hash alone is sufficient to
       recognize this computer across check-ins. Each gets its OWN unique
       key (not a combined one) so that either value alone can identify an
       existing row - MySQL does not enforce uniqueness among NULLs, so
       many rows can share machine_id=NULL (or device_public_key_hash=NULL)
       without conflict. */
